...
export const SPEAKER_GROUPS = [
  ...
  // 'Z' = Azan (call-to-prayer) minaret horn array — exterior, own amp/zone.
  // Future special groups can claim other letters here.
  { id: 'I', label: 'Imam mic', color: '#fbbf24' },
  { id: 'Z', label: 'Azan horns', color: '#06b6d4' },
];
...
export const state = {
  ...
  display: {
    ...
    // default) / 'high' (0.25 m) / 'ultra' (0.125 m) — finer cells = more
    // sampling detail, at ~(cells)² more compute. Applied to the 2D viewport,
    // the 3D heatmap, and the print report (all sample through computeSPLGrid;
    // the 3D GPU shader smooths on top). Session-only.
    heatmapRes: 'standard',
  },
