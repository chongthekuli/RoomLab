...
export const PRESETS = {
  ...
  //
  // Scaled-down model of the real mall (which is ~340×90 m over 5 retail
  // levels) — 80 × 40 m footprint, 4 levels at 5.8 m floor-to-floor, with
  // a central atrium void punched through every slab. Structural columns
  // on a 10 × 10 m grid (real Malaysian super-regional malls use 9-10 m).
  // Ceiling speakers follow Malaysian BOMBA voice-alarm norms: Amperes
  // CS610B (10 W 100 V, 92 dB/1W/1m) on a ~10 m grid — 1 speaker per
  // 100 m² concourse delivers ≥ 65 dBA with STI ≥ 0.5.
  //
  // Phase 1 (this preset): full static architecture, dense PA layout,
  // listeners scattered across all four levels. Walkthrough is limited
  // to the ground floor — escalator elevation transitions are Phase 2.
  // ------------------------------------------------------------------
  pavilion: (() => {
    const W = 80, D = 40;
    ...
    // 10 × 10 m column grid, 900 mm square RC columns (rendered as 450
    // mm radius cylinders — close enough visually). Skip columns that
    // fall inside the atrium footprint.
    const columns = [];
    ...
    // Three floor slabs (ground is room.floor — implicit). Each slab is
    // the footprint minus the atrium, extruded by slab_thickness_m at
    // the given elevation.
    const levels = [];
    for (let i = 1; i < nLevels; i++) {
      levels.push({
        index: i,
        slab_z: i * levelHeight,           // top surface of the slab = floor of level i
    ...
    for (let lv = 0; lv < nLevels - 1; lv++) {
      ...
      // North-side escalator (up-side goes from atrium NE corner toward NW)
      escalators.push({
      ...
      // South-side escalator (scissor — opposite direction)
      escalators.push({
        from_level: lv, to_level: lv + 1,
    ...
    // Build the ceiling-speaker grid. Amperes CS610B on 10 m centres on
    // L0 (busiest concourse), 12 m centres on upper levels (quieter
    // retail). Speakers hang ~0.1 m below the slab above.
    const SPKR_MODEL = 'data/loudspeakers/amperes-cs610b.json';
    const sources = [];
    ...
    // Listeners — 2-3 characteristic positions on each level so the user
    // can audit PA coverage / STI everywhere.
    const listeners = [
      // Ground floor (L0)
    ...
    return {
      ...
      // Material palette — polished concrete / ceramic floors, gypsum
      // partitions, acoustic-tile suspended ceilings. Highly
      // reverberant overall — part of the point of using the mall as a
      // PA-coverage torture test.
      surfaces: {
        floor: 'concrete',
        ceiling: 'acoustic-tile',
        walls: 'gypsum-board',
