...
| Furniture / rack absorption & blocking | `furniture-absorption.js`, `rack-absorption.js`, `*-direct-blocking.js`, `furniture-sub-volumes.js` | ISO 354; Beer-Lambert μ=A/4V | **TODO.** Articulate the absorption + direct-shadowing model. **Audience block (2026-05-31):** the occupied-audience flat-pad sub-volume is a porous COLUMN to ~0.92·h (not a 4 cm mat) so the Beer-Lambert direct-shadow acts at seated-ear height (~0.85 dB grazing loss per 1 m block @ 1 kHz), reconciling the analytical helper with the precision tracer (which already uses the full footprint volume). A_obj is unchanged → RT60 unaffected. This is BULK grazing absorption, **not** the seat-dip effect (the deep ~100–300 Hz / 10–15 dB low-mid notch from periodic seat-row interference, Schultz-Watters 1964 / ISO 9613-2 §7.5) — that needs row-spacing geometry this tier lacks (P3 backlog); expect under-prediction of LF loss over a deep raked audience, no notch. |
